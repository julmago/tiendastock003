...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if (!$valid) $err="Elegí un medio de pago válido.";
    ...
    foreach($cart as $pid=>$qty){
      ...
      // Cambio clave: stock minorista solo con inventario propio.
    ...
    if (empty($err) && $itemsTotal>0) {
      ...
      try {
        ...
        $sid = $pdo->prepare("SELECT retailer_id FROM stores WHERE id=?");
        $sid->execute([(int)$store['id']]);
        $retailerId = (int)($sid->fetch()['retailer_id'] ?? 0);

        $pdo->prepare("INSERT INTO fees_ledger(order_id,store_id,wholesaler_id,retailer_id,provider_id,fee_type,amount) VALUES(?,?,?,?,?,?,?)")
            ->execute([$orderId,(int)$store['id'],NULL,$retailerId,NULL,'seller_fee',$sellerFee]);
        if ($mpExtra>0) {
          $pdo->prepare("INSERT INTO fees_ledger(order_id,store_id,wholesaler_id,retailer_id,provider_id,fee_type,amount) VALUES(?,?,?,?,?,?,?)")
              ->execute([$orderId,(int)$store['id'],NULL,$retailerId,NULL,'mp_extra',$mpExtra]);
        }
