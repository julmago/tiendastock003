...
// Cambio clave: login separado para rol minorista.
if ($_SERVER['REQUEST_METHOD']==='POST') {
  $u = login_user($pdo, $_POST['email']??'', $_POST['password']??'', ['retailer']);
  if ($u) { session_set_user($u); header('Location: /minorista/'); exit; }
  $err="Credenciales inválidas.";
}
page_header('Minorista - Login');
