...
$newUrl = "productos.php?action=new";
...
page_header('Minorista - Productos');
// Cambio clave: catálogo minorista separado del mayorista/proveedor.
...
echo "<div style='display:flex; align-items:center; justify-content:space-between; gap:12px;'>
  <h2 style='margin:0;'>Minorista - Productos</h2>
...
if ($action === 'list') {
  ...
  $stp->execute([(int)$currentStore['id']]);
  ...
  <tr><th>ID</th><th>Título</th><th>SKU</th><th>Código universal</th><th>Own qty</th><th>Own $</th><th>Manual $</th><th>Precio actual</th></tr>";
  foreach($storeProducts as $sp){
    $sell = current_sell_price($pdo, $currentStore, $sp);
    $stockTotal = (int)$sp['own_stock_qty'];
    $sellTxt = ($sell>0) ? '$'.number_format($sell,2,',','.') : 'Sin stock';

    $editUrl = "producto.php?id=".h((string)$sp['id']);
    echo "<tr>
...
if ($action === 'new') {
  $providerQuery = trim((string)($_GET['wholesaler_q'] ?? ''));
  $wholesalerProducts = [];
  ...
  if ($providerQuery !== '' && $whStore) {
    ...
    $searchSt = $pdo->prepare("
      SELECT sp.id, sp.title, sp.sku, sp.universal_code, sp.own_stock_price, sp.description, s.name AS wholesaler_name,
             sp.own_stock_qty AS stock
      FROM store_products sp
      JOIN stores s ON s.id = sp.store_id
      WHERE sp.status='active' AND s.id=?
        AND (sp.title LIKE ? OR sp.sku LIKE ? OR sp.universal_code LIKE ?)
      ORDER BY sp.id DESC
      LIMIT 20
    ");
    $searchSt->execute([(int)$whStore['id'], $like, $like, $like]);
  ...
  if ($wholesalerProducts) {
    ...
    foreach ($rows as $row) {
      ...
      if (!isset($wholesalerProductImages[$pid])) {
        $wholesalerProductImages[$pid] = [];
      }
      $wholesalerProductImages[$pid][] = [
  ...
  echo "<h3>Copiar desde mayorista</h3>";
  if (!$whStore) {
    echo "<p>Primero vinculá un mayorista para poder copiar productos.</p>";
  } else {
    echo "<p>Mayorista vinculado: <b>".h($whStore['name'])."</b></p>
    <form method='get' action='productos.php'>
    <input type='hidden' name='action' value='new'>
    <input name='wholesaler_q' placeholder='Buscar producto del mayorista...' value='".h($providerQuery)."' style='width:420px'>
  ...
  (function() {
    ...
    buttons.forEach(function(button) {
      button.addEventListener('click', function() {
        ...
        updateOrder();
        if (message) {
          message.textContent = 'Datos cargados desde mayorista. Revisá y presioná Crear para publicar.';
