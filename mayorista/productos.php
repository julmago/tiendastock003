...
$storeSt = $pdo->prepare("SELECT id, name, slug, store_type, markup_percent FROM stores WHERE wholesaler_id=? AND store_type='wholesale' LIMIT 1");
$storeSt->execute([(int)$wholesaler['id']]);
$currentStore = $storeSt->fetch();
if (!$currentStore) { page_header('Productos'); echo "<p>Primero creá tu tienda mayorista.</p>"; page_footer(); exit; }
...
page_header('Mayorista - Productos');
// Cambio clave: catálogo mayorista copia desde proveedores vinculados.
...
echo "<div style='display:flex; align-items:center; justify-content:space-between; gap:12px;'>
  <h2 style='margin:0;'>Mayorista - Productos</h2>
...
if ($action === 'new') {
  ...
  $linkedProviders = $pdo->prepare("SELECT provider_id FROM wholesaler_provider WHERE wholesaler_id=?");
  $linkedProviders->execute([(int)$wholesaler['id']]);
  $providerIds = array_map('intval', $linkedProviders->fetchAll(PDO::FETCH_COLUMN));

  if ($providerQuery !== '' && $providerIds) {
    $like = "%{$providerQuery}%";
    $placeholders = implode(',', array_fill(0, count($providerIds), '?'));
    $params = array_merge($providerIds, [$like, $like, $like]);
  ...
  if (!$providerIds) {
    echo "<p>Primero vinculá al menos un proveedor.</p>";
  } else {
    echo "<form method='get' action='productos.php'>
