...
$productSt = $pdo->prepare("SELECT sp.*, s.name AS store_name, s.store_type, s.markup_percent, s.id AS store_id
  ...
  WHERE sp.id=? AND s.wholesaler_id=? LIMIT 1");
$productSt->execute([$productId,(int)$wholesaler['id']]);
...
(function() {
  ...
  function fetchResults(query) {
    ...
    fetch('/mayorista/api/provider_products_search.php?' + params.toString(), {
  ...
  function linkProviderProduct(linkedId, rowEl) {
    ...
    fetch('/mayorista/api/link_product.php', {
      method: 'POST',
      headers: {
